...
function DatePicker( picker, settings ) {
    ...
    // pgraffam: add in levels
    calendar.item.levels = settings.levels;

    // pgraffam: add in prices
    calendar.item.prices = settings.prices;
...
/**
 * pgraffam: custom levels function.
 */
DatePicker.prototype.levels = function( dateObject ) {

  var calendar = this,
      level = '';

  if (calendar.item.levels) {

    $.each(calendar.item.levels, function(index) {
      //console.log(index);

      this.filter( function( dateToLevel ) {

        if( $.isArray( dateToLevel ) ) {
          if (dateObject.pick === calendar.create( dateToLevel ).pick) {
            level = index;
          }
        }
      });
...
DatePicker.prototype.nodes = function( isOpen ) {
    ...
    var
        ...
        levelsCollection = calendar.item.levels, //pgraffam
        pricesCollection = calendar.item.prices, //pgraffam
        minLimitObject = calendar.item.min,
        maxLimitObject = calendar.item.max,
        ...
        if (levelsCollection) {
          if(pricesCollection) {
            if (calendar.item.prices) {
              $.each(calendar.item.prices, function(index, value) {
                prices[index] = value;
              });

              footerDiv = '<ul>' +
                '<li><span class="color-box level1"></span> Standard Service Fees</li>' +
                '<li><span class="color-box level2"></span> +$' + prices[0] + '</li>' +
                '<li><span class="color-box level3"></span> +$' + prices[1] + '</li>' +
                '<li><span class="color-box level4"></span> +$' + prices[2] + '</li>' +
                '<li><span class="color-box level5"></span> +$' + prices[3] + '</li>' +
              '</ul>' +
              '<a href="#" data-close="1" class="closer">Close</a>';
            }
          } else {
            footerDiv = '<ul>' +
              '<li><span class="color-box level1"></span> Standard Service Fees</li>' +
              '<li><span class="color-box level2"></span> +$2.99</li>' +
              '<li><span class="color-box level3"></span> +$3.99</li>' +
              '<li><span class="color-box level4"></span> +$4.99</li>' +
              '<li><span class="color-box level5"></span> +$5.99</li>' +
            '</ul>' +
            '<a href="#" data-close="1" class="closer">Close</a>';
          }
        } else {
          footerDiv = '<a href="#" data-close="1" class="closer">Close</a>';
        }
    ...
    // pgraffam: Create and return the entire calendar.
    return Picker._.node(
        'div',
        createMonthNav() +
                                ...
                                return [
                                    Picker._.node(
                                        ...
                                        (function( klasses ) {
                                            ...
                                            // pgraffam: set up levels if there are any and they fall within range
                                            var level = calendar.levels( targetDate );

                                            if ( levelsCollection && level || targetDate.pick < minLimitObject.pick || targetDate.pick > maxLimitObject.pick ) {
                                                var levelNumber = 'level' + level

                                                switch(levelNumber) {
                                                  case 'level5':
                                                    klasses.push( settings.klass.level5 )
                                                    break;
                                                  case 'level4':
                                                    klasses.push( settings.klass.level4 )
                                                    break;
                                                  case 'level3':
                                                    klasses.push( settings.klass.level3 )
                                                    break;
                                                  case 'level2':
                                                    klasses.push( settings.klass.level2 )
                                                    break;
                                                }
                                            }
...
DatePicker.defaults = (function( prefix ) {
    ...
    return {
        ...
        klass: {
            ...
            // pgraffam: levels classes
            level5: prefix + 'day--level5',
            level4: prefix + 'day--level4',
            level3: prefix + 'day--level3',
            level2: prefix + 'day--level2',
