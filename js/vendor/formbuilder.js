...
(function() {
  ...
  ViewFieldView = (function(_super) {
    ...
    ViewFieldView.prototype.render = function() {
      ...
      //   console.log(options);
      //   function objectFilter(value){
      //     return (typeof value === 'object');
      //   }
      //   var filteredOptions = options.filter(objectFilter);
      //   console.log(filteredOptions);

      //   // this.model.set(Formbuilder.options.mappings.OPTIONS, '[]');
        // console.log(options);
        
        // this.model.set(Formbuilder.options.mappings.OPTIONS, filteredOptions);
        // console.log(options);
  ...
  EditFieldView = (function(_super) {
    ...
    EditFieldView.prototype.initialize = function(options) {
      this.parentView = options.parentView;
      this.rightView = this.parentView.$el.find('.fb-right');
      return this.listenTo(this.model, "destroy", this.remove);
    };
    ...
    EditFieldView.prototype.render = function() {
      ...
      var mid = this.model.get('cid');

      edited = this.rightView.find('.fb-field-wrapper[data-cid="'+mid+'"]');


      // eft = this.$el.find('.field-type');                     // eft =  Field type
      // efl = this.$el.find('.field-label');                    // efl =  Label
      // efl2 = this.$el.find('.fb-label-description input');    // efl2 = Long description area
      // efr = this.$el.find('.fb-common-checkboxes input');     // efr =  Required checkbox
      // efs = this.$el.find('.fb-size-select');                 // efs =  Size dropdown
      
      // rft = $(edited).find('label span');

      // m = {
      //   field_type: this.model.get('field_type'),
      //   field_options: this.model.get('field_options'),
      ...
      //   var filteredOptions = options.filter(objectFilter);
       ...
       // this.model.set(Formbuilder.options.mappings.OPTIONS, '[]');
        //console.log(this.model.get(Formbuilder.options.mappings.OPTIONS));
      ...
      // var filteredOptions = options.filter(objectFilter);
      // console.log(filteredOptions);
      // foptions = Formbuilder.options.mappings.OPTIONS;
      // this.model.set({
      ...
      // efo = this.$el.find('.fb-edit-section-options');

      // rivets.bind($(efo), {
      //   model: this.model.get('field_options')
      // });
  ...
  BuilderView = (function(_super) {
    ...
    BuilderView.prototype.createAndShowEditView = function(model) {
      ...
      $responseFieldEl.addClass('editing').attr('data-cid',model.get('cid')).siblings('.fb-field-wrapper').removeClass('editing');
      if (this.editView) {
  ...
  Formbuilder = (function() {
    ...
    Formbuilder.filteredOptions = function(rf) {
        var options = rf.get(Formbuilder.options.mappings.OPTIONS);
        function objectFilter(value){
          return (typeof value === 'object');
        }
        var filteredOs = options.filter(objectFilter);
      return filteredOs;
    };
...
// (function() {
//   Formbuilder.registerField('custom text', {
//     order: 55,
//     type: 'non_input',
//     view: "<label class='section-name'><%= rf.get(Formbuilder.options.mappings.LABEL) %></label>\n<p><%= rf.get(Formbuilder.options.mappings.CUSTOMTEXT) %></p>",
//     edit: "<%= Formbuilder.templates['edit/customtext']() %>",
//     addButton: "<span class='symbol'><span class='fa fa-minus'></span></span> Custom Text",
//     simple_format: true,
//     defaultAttributes: function(attrs) {
//       var attrs;
//       attrs = {};
//       attrs.label = '';
//       attrs[Formbuilder.options.mappings.FIELD_TYPE] = 'Custom Text';
//       attrs[Formbuilder.options.mappings.CUSTOMTEXT] = 'p';
//       return attrs;
//     }
//       // defaultFieldAttrs: function(field_type) {
//       //   var attrs, _base;
...
var __t, __p = '', __e = _.escape;
with (obj) {
__p += '<div class=\'fb-field-label\'>\n  <span class=\'field-label\' rv-text="model.' +
...
// ((__t = ( Formbuilder.templates['edit/base_header']() )) == null ? '' : __t) +
// '\n' +
// ((__t = ( Formbuilder.fields[rf.get(Formbuilder.options.mappings.LABEL)].edit({rf: rf}) )) == null ? '' : __t) +
// '\n';
...
var __t, __p = '', __e = _.escape;
with (obj) {
__p += '<input type=\'text\' rv-value=\'model.label\' />\n<textarea rv-input=\'model.' +
...
// this["Formbuilder"]["templates"]["edit/customtext"] = function(obj) {
// obj || (obj = {});
// var __t, __p = '', __e = _.escape;
// with (obj) {
// __p += '<div class=\'fb-edit-section-header\'>Custom Text Content</div>\n\n<textarea rv-input=\'model.' +
// ((__t = ( Formbuilder.options.mappings.CUSTOMTEXT )) == null ? '' : __t) +
// '\'\n ></textarea>';
...
with (obj) {
__p += '<div class=\'fb-edit-section-options\'><div class=\'fb-edit-section-header\'>Options</div>\n\n';
